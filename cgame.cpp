...
{
 static const char RESPAWN[]="RESPAWN";//имя тайла восстановления
...
void CGame::DrawScreenFrame(IVideo *iVideo_Ptr)
{
 ...
 if (x2>x_bad)
 {
  int32_t x_normal=x1+part*2;//вторая часть
...
bool CGame::LoadConditionalFile(const std::string &file_name,std::vector<std::string> &log)
{
 ...
 if (file!=NULL)
 {
  ...
  while(done==false)
  {
   ...
   {
    char str[255];
    sprintf(str,"%i",line_index);
   	if (line.length()!=0) log.push_back(std::string(str)+": "+line);
	line_index++;
    line="";
   }
  }
...
//обработка таймера
//----------------------------------------------------------------------------------------------------
bool CGame::OnTimer(bool left,bool right,bool up,bool down,bool fire,IVideo *iVideo_Ptr)
{
 bool no_change_page=false;
